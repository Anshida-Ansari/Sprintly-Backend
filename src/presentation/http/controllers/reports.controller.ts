...
export class ReportsController {
		...
		return {
			...
			projectId:
				req.params.projectId || req.query.projectId
					? String(req.params.projectId || req.query.projectId)
					: undefined,
			sprintId:
				req.params.sprintId || req.query.sprintId
					? String(req.params.sprintId || req.query.sprintId)
					: undefined,
			userId:
				req.params.userId || req.query.userId
					? String(req.params.userId || req.query.userId)
					: undefined,
	...
	async getProjectReports(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const result = await this.getProjectReportsUseCase.execute(
				companyId,
				filters,
			);
	...
	async getSprintReports(req: Request, res: Response, next: NextFunction) {
		try {
			const companyId = req.user.companyId;
			const filters = this.getFilters(req);
			const result = await this.getSprintReportsUseCase.execute(
				companyId,
				filters,
			);
	...
	async getUserStoryReports(req: Request, res: Response, next: NextFunction) {
		try {
			const companyId = req.user.companyId;
			const filters = this.getFilters(req);
			const result = await this.getUserStoryReportsUseCase.execute(
				companyId,
				filters,
			);
	...
	async getSubtaskReports(req: Request, res: Response, next: NextFunction) {
		try {
			const companyId = req.user.companyId;
			const filters = this.getFilters(req);
			const result = await this.getSubtaskReportsUseCase.execute(
				companyId,
				filters,
			);
	...
	async getUserPerformanceReports(
		...
		try {
			const companyId = req.user.companyId;
			const filters = this.getFilters(req);
			const result = await this.getUserPerformanceReportsUseCase.execute(
				companyId,
				filters,
			);
