...
export class SuperAdminController {
	...
	async getRevenueAnalytics(_req: Request, res: Response, next: NextFunction) {
		try {
	...
	async getTopCompanies(_req: Request, res: Response, next: NextFunction) {
		try {
	...
	async getSubscriptionReport(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const result = await this._getSubscriptionReportsUseCase.getSubscriptions(
				page,
				limit,
			);
	...
	async getPaymentReport(req: Request, res: Response, next: NextFunction) {
		try {
			const page = Number(req.query.page) || 1;
			const limit = Number(req.query.limit) || 10;
			const result = await this._getSubscriptionReportsUseCase.getPayments(
				page,
				limit,
			);
			return res.status(SuccessStatus.OK).json({ success: true, ...result });
	...
	async getExpiringSoonReport(req: Request, res: Response, next: NextFunction) {
		try {
			const page = Number(req.query.page) || 1;
			const limit = Number(req.query.limit) || 10;
			const result = await this._getSubscriptionReportsUseCase.getExpiringSoon(
				page,
				limit,
			);
			return res.status(SuccessStatus.OK).json({ success: true, ...result });
	...
	async getTrialReport(req: Request, res: Response, next: NextFunction) {
		try {
			const page = Number(req.query.page) || 1;
			const limit = Number(req.query.limit) || 10;
			const result = await this._getSubscriptionReportsUseCase.getTrials(
				page,
				limit,
			);
			return res.status(SuccessStatus.OK).json({ success: true, ...result });
