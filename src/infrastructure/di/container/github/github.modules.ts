...
export const GitHubModule = new ContainerModule(({bind}) => {
    bind<IGitHubOAuthService>(GITHUB_TYPE.IGitHubOAuthService).to(GitHubOAuthService)
    

    bind<IGitHubRepositoryService>(GITHUB_TYPE.IGitHubRepositoryService).to(GitHubRepositoryService)
       
    bind<IConnectGitHubUseCase>(GITHUB_TYPE.IConnectGitHubUseCase).to(ConnectGitHubUseCase)

    bind<IDisconnectGitHubUseCase>(GITHUB_TYPE.IDisconnectGitHubUseCase).to(DisconnectGitHubUseCase)

    bind<IGetGitHubStatusUseCase>(GITHUB_TYPE.IGetGitHubStatusUseCase).to(GetGitHubStatusUseCase)
