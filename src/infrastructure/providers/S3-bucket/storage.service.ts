...
export class StorageService implements IStorageService {
	...
	constructor() {
		...
		if (!region || !accessKeyId || !secretAccessKey || !this.bucketName) {
			logger.error(
				"FATAL ERROR: AWS Credentials or Region are missing in environment variables.",
			);
	...
	async generateUploadSignedUrl(
		...
		try {
			...
			logger.error({ err: error }, "S3 Generate Signed URL failed");
			throw error;
		}
	}
	...
	async generateDownloadSignedUrl(fileUrl: string): Promise<string> {
		try {
			...
			logger.error({ err: error }, "S3 Generate Download Signed URL failed");
			throw error;
		}
	}
}
