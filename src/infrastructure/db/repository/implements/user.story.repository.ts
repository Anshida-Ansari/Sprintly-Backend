...
import type { IUserStoryRepository } from "../interface/user.story.interface";
	...
	implements IUserStoryRepository
{
	...
	async findByIds(ids: string[]): Promise<UserStoryEntity[]> {
	...
	async addComment(
		...
		await this.model.findByIdAndUpdate(userStoryId, {
			$push: {
				comments: comment,
			},
		});
