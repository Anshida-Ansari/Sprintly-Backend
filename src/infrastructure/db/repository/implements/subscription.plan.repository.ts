...
export class SubscriptionPlanRepository implements ISubscriptionPlanRepository {
	...
	async create(data: {
		...
		const entity = this._mapper.toEntity(newPlan.toObject() as Record<string, unknown>);
	...
	async update(id: string, data: Partial<SubscriptionPlanEntity>): Promise<SubscriptionPlanEntity | null> {
		...
		return updated ? this._mapper.toEntity(updated.toObject() as Record<string, unknown>) : null;
	}

	async findById(id: string): Promise<SubscriptionPlanEntity | null> {
		const plan = await SubscriptionPlanModel.findById(id);
		return plan ? this._mapper.toEntity(plan.toObject() as Record<string, unknown>) : null;
	}
	...
	async findAll(filter: Record<string, unknown> = {}): Promise<SubscriptionPlanEntity[]> {
