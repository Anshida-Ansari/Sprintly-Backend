...
import type {
	IAiDataAggregator,
	ProjectContext,
} from "../interface/ai.data-aggregator.interface";
	...
	async getProjectContext(
		...
		const blockedTasks = allSubtasks.filter((st) => {
			const isBlockedStatus =
				(st.status as string).toLowerCase() === "blocked" ||
				(st.status as string).toLowerCase() === "stuck";
			const hasBlockedComment = st.comments?.some((c) =>
				c.message.toLowerCase().includes("block"),
			);
			return isBlockedStatus || hasBlockedComment;
		});

		const completedTasksCount = allSubtasks.filter(
			(st) => st.status === SubTaskStatus.COMPLETED,
		).length;
		const pendingTasksCount = allSubtasks.filter(
