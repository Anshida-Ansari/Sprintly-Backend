...
import { IGitHubOAuthService } from "@domain/interface/github.oauth.interface";
...
export class ConnectGitHubUseCase implements IConnectGitHubUseCase {
    ...
    async execute(dto: ConnectGitHubDTO, companyId: string,): Promise<GitHubStatusResponse> {
        const company = await this._companyRepository.findById(companyId);
        if (!company) {
            throw new NotFoundError("Company not found");
        }
        const tokens = await this._githubOAuthService.exchangeCodeForToken(dto.code);

        const userInfo = await this._githubOAuthService.getUserInfo(tokens.accessToken,);
        ...
        const encryptedAccessToken = EncryptionUtil.encrypt(tokens.accessToken, encryptionKey,);
