...
@injectable()
export class GetDeveloperDashboardStatsUseCase implements IGetDeveloperDashboardStatsUseCase {
	constructor(
	...
	async execute(userId: string, companyId: string): Promise<IDeveloperDashboardStats> {
		const allAssignedSubTasks = await this._subtaskRepository.findByAssignedTo(userId);
		
		const userSubTasks = allAssignedSubTasks.filter(t => t.companyId.toString() === companyId.toString());

		let currentFocus = null;
		let inProgressTasks = userSubTasks.filter(t => t.status === SubTaskStatus.IN_PROGRESS);
		
		if (inProgressTasks.length === 0) {
			inProgressTasks = userSubTasks.filter(t => t.status === SubTaskStatus.PENDING);
		}
		...
		const upNextRaw = userSubTasks.filter(t => t.status === SubTaskStatus.PENDING && t.id !== currentFocus?.id);
		...
		const allMeetings = await this._meetingRepository.find({ projectId: { $in: projectIds } }, { skip: 0, limit: 100 });
		
		const today = new Date();
		const schedule = allMeetings
			.filter(m => {
				const isParticipant = m.participants?.some((p: any) => p.userId?.toString() === userId.toString()) || m.createdBy.toString() === userId.toString();
				const mDate = new Date(m.date);
				const isToday = mDate.getDate() === today.getDate() && mDate.getMonth() === today.getMonth() && mDate.getFullYear() === today.getFullYear();
		...
		const completedTasks = userSubTasks.filter(t => t.status === SubTaskStatus.COMPLETED).length;
		const totalTasks = userSubTasks.length || 1;

		const loggedHours = userSubTasks.reduce((acc, curr) => acc + (curr.actualHours || 0), 0);
		const totalHours = userSubTasks.reduce((acc, curr) => acc + (curr.estimatedHours || 0), 0) || 1;
